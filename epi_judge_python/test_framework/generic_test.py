...
def generic_test_main(
    test_file, test_data_file, test_func, comparator=None, res_printer=None
):
    """
...
def run_tests(handler, config, res_printer):
    ...
    for test_case in test_data[1:]:
        ...
        print_test_info(
            result, test_nr, total_tests, test_failure.get_description(), test_timer
        )
        ...
        if result != TestResult.PASSED:
            if config.verbose:
                ...
                if test_explanation not in ("", "TODO"):
                    test_failure.with_property(
                        PropertyName.EXPLANATION, test_explanation
                    )
                print_failed_test(
